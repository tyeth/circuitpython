...
//|           ``bit_clock`` and ``word_select`` are inputs driven by something else, another I2S
//|           object (the clock source), or a codec, rather than generated by this object. Not
//|           supported on all ports.
//|
//|           In external clock mode ``bit_clock`` and ``word_select`` do not have to be sequential
//|           GPIOs, and they may be shared with another I2S object. `play` cannot retime the bus, so
//|           the sample's ``sample_rate`` must equal the external frame rate or the pitch will be
//|           wrong. If the incoming clock stops, output stalls with its last sample held.
//|
...
static mp_obj_t audiobusio_i2sout_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args) {
    ...
    #else
    enum { ARG_bit_clock, ARG_word_select, ARG_data, ARG_main_clock, ARG_left_justified, ARG_external_clock };
    ...
    common_hal_audiobusio_i2sout_construct(self, bit_clock, word_select, data, main_clock,
        args[ARG_left_justified].u_bool, external_clock);
